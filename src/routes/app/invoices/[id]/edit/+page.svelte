...
<div class="max-w-3xl space-y-8">
  ...
  <div class="flex items-start justify-between">
    <div>
      <h1 class="text-lg font-medium mb-1">Edit Invoice {data.invoice?.number || ''}</h1>
      <!-- Breadcrumbs -->
      <div class="flex items-center gap-2 text-xs">
        <a href="/app/invoices" class="text-gray-400 hover:text-black transition-colors">Invoices</a>
        <span class="text-gray-400">/</span>
        <a href="/app/invoices/{data.invoice?.id}" class="text-gray-400 hover:text-black transition-colors">{data.invoice?.number}</a>
        <span class="text-gray-400">/</span>
