<script lang="ts">
  ...
  let selectedTemplateId = $state(data.invoice?.template_id || '');
  let issueDate = $state(data.invoice?.issue_date || new Date().toISOString().split('T')[0]);
  ...
  let lineItems = $state<any[]>(
    data.invoice?.items && data.invoice.items.length > 0
      ? data.invoice.items.map((item: any) => ({
          ...
          taxRate: item.tax_rate * 100 // Convert from decimal to percentage
        }))
  ...
  // Auto-select first template if none selected (only for new invoices)
  $effect(() => {
    if (data.templates && data.templates.length > 0 && !selectedTemplateId && !data.invoice) {
      selectedTemplateId = data.templates[0].id;
    }
  });

  // Update currency when client changes (only for new invoices)
  $effect(() => {
    if (selectedClient && selectedClient.currency && !data.invoice) {
...
<svelte:head>
  <title>Edit Invoice {data.invoice?.number || ''} - inv</title>
</svelte:head>
...
<div class="max-w-3xl space-y-8">
  ...
  <div class="flex items-start justify-between">
    <div>
      <h1 class="text-lg font-medium mb-1 dark:text-white">Edit Invoice {data.invoice?.number || ''}</h1>
      <!-- Breadcrumbs -->
      <div class="flex items-center gap-2 text-xs">
        <a href="/app/invoices" class="text-gray-400 hover:text-black dark:hover:text-white transition-colors">Invoices</a>
        <span class="text-gray-400">/</span>
        <a href="/app/invoices/{data.invoice?.id}" class="text-gray-400 hover:text-black dark:hover:text-white transition-colors">{data.invoice?.number}</a>
        <span class="text-gray-400">/</span>
      ...
      <div class="space-y-5">
        <div class="flex items-center justify-between pb-2 border-b border-gray-200 dark:border-gray-700">
          ...
          <button
            type="button"
            onclick={addLineItem}
            class="px-5 py-2.5 text-sm text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white transition-colors"
          >
            Add Item
          </button>
        </div>
                ...
                <div class="md:col-span-2">
                  {#if lineItems.length > 1}
                    <button
                      ...
                      class="w-full px-2 py-2.5 text-sm text-gray-500 dark:text-gray-400 hover:text-red-600 dark:hover:text-red-400 transition-colors"
                    >
                      Remove
                    </button>
                  {/if}
                  ...
                  <div class="flex items-end">
                    {#if lineItems.length > 1}
                      <button
                        type="button"
                        onclick={() => removeLineItem(index)}
                        class="w-full px-4 py-2.5 text-sm text-gray-600 dark:text-gray-400 hover:text-red-600 dark:hover:text-red-400 border border-gray-300 dark:border-gray-600 hover:border-red-300 dark:hover:border-red-400 transition-colors"
                      >
                        Remove
                      </button>
                    {/if}
    ...
    <div class="bg-white dark:bg-dark-bg w-full h-full md:w-[90vw] md:h-[90vh] md:max-w-5xl flex flex-col" onclick={(e) => e.stopPropagation()}>
      ...
      <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
        ...
        <button
          ...
          class="px-3 py-1.5 text-xs text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white transition-colors"
        >
          Close
        </button>
      </div>
