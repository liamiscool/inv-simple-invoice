...
function Particle(x, y) {
  ...
  this.move = function() {
    ...
    if (this.distToTarget < closeEnoughSlider.slider.value()) {
      var proximityMult = this.distToTarget/closeEnoughSlider.slider.value();
      this.vel.mult(0.9);
    } else {
      var proximityMult = 1;
      this.vel.mult(0.95);
  ...
  this.draw = function() {
    ...
    if (! this.isKilled) {
      ...
      var targetSize = map(min(this.distToTarget, closeEnoughSlider.slider.value()), 
                           closeEnoughSlider.slider.value(), 0, 
    ...
    if (particleShape === 'circle') {
      ...
      var h = this.currentSize * 0.866; // height of equilateral triangle
      triangle(0, -h/2, -this.currentSize/2, h/2, this.currentSize/2, h/2);
    }
  ...
  this.isOutOfBounds = function() {
    if (boundaryShape === 'circle') {
      // Check if particle is outside circular boundary
      var centerX = width / 2;
      var centerY = height / 2;
      var radius = min(width, height) / 2;
      var distFromCenter = dist(this.pos.x, this.pos.y, centerX, centerY);
      return distFromCenter > radius;
    } else {
      // Check if particle is outside square boundary
